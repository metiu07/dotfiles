...
class Installer:
    ...
    def __init__(self, args):
        ...
        if not self.install_mode:
            ...
            if not self.confirm('Do you want to proceed?', default=False, auto_confirm=True):
    ...
    def load_configuration(self, input_file: str) -> Dict[str, str]:
        ...
        try:
            ...
            sys.exit('Input file "{}" can not be parsed.\n{}'.format(input_file, str(err)))
        ...
        for m in modules:
            ...
            if self.install_mode and 'install' in c:
                ...
                if os.path.exists(install_script):
                    print('Target "{}" wants to run script "{}"'.format(m, install_script))
                    ...
                    if not self.confirm('Install "{}"?'.format(m)):
                        sys.exit('Aborting... Install script for "{}" @ "{}" was not allowed to run.'.format(m, c['install']))
                    self.exec_command([c['install']])
                else:
                    sys.exit('Aborting... Install script for "{}" @ "{}" does not exist.'.format(m, c['install']))
    ...
    def install(self, module):
        ...
        os.makedirs(os.path.dirname(self.prepare_path(module['destination'])), exist_ok=True)
        self.exec_command(self.install_cmd + [self.prepare_path(module['source']), self.prepare_path(module['destination'])])
    ...
    def exec_command(self, c: List[str], check=True, **kwargs) -> subprocess.CompletedProcess:
    ...
    def confirm(self, s: str, default: bool=True, auto_confirm: Optional[bool]=None) -> bool:
...
def parse_arguments():
    ...
    parser.add_argument('-a', '--auto-confirm', action='store_true',
                        help='skip all user confirmations')
